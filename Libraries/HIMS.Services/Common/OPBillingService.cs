    ...
    public class OPBillingService : IOPBillingService
    {
            ...
            try
            {
                ...
                foreach (var objItem1 in objBill.AddCharges)
                {
                        ...
                        if (objItem1.IsPackage == 1)
                        {
                            foreach (var item in ObjaddCharge)
                            {
                                string[] AEntity = { "ChargesId", "ChargesDate", "OpdIpdType", "OpdIpdId", "ServiceId", "Price", "Qty", "UnitId", "TotalAmt", "ConcessionPercentage", "ConcessionAmount", "NetAmount", "DoctorId", "DoctorName", "DocPercentage", "DocAmt", "HospitalAmt", "RefundAmount", "IsGenerated", "IsComServ", "IsPrintCompSer", "AddedBy", "IsCancelled", "IsCancelledBy", "IsCancelledDate", "IsPathology", "IsRadiology", "IsPackage", "WardId", "BedId", "ServiceCode", "ServiceName", "CompanyServiceName", "IsInclusionExclusion", "IsHospMrk", "PackageMainChargeID", "IsSelfOrCompanyService", "PackageId", "ChargesTime", "ClassId", "TariffId", "BillNo", "CreatedBy", "IsOtherService" };
                                var Packagescharge = item.ToDictionary();
                    ...
                    if (ObjTDrbill != null && ObjTDrbill.Drbno > 0)
                    {
                        ...
                        odal.ExecuteNonQuery("PS_UpdateDraft", CommandType.StoredProcedure, tentity);
                       await _context.LogProcedureExecution(  tentity,  nameof(TDrbill),  ObjTDrbill.Drbno.ToInt(),  Core.Domain.Logging.LogAction.Edit,  CurrentUserId,   CurrentUserName);
                    }
                    // Save Log
        ...
        public virtual async Task AppBillInsert(Registration objRegistration, VisitDetail objVisitDetail, Bill objBill, Payment objPayment, List<AddCharge> ObjaddCharge, int CurrentUserId, string CurrentUserName)
        {
            ...
            try
            {
                ...
                odal1.SetConnection(_context.Database.GetDbConnection()); // <-- Share same DbConnection
                ...
                string RegId = odal1.ExecuteNonQueryNew("ps_insert_Registration_1", CommandType.StoredProcedure, "RegId", entity);
                ...
                string VisitId = odal1.ExecuteNonQueryNew("ps_insert_VisitDetails_1", CommandType.StoredProcedure, "VisitId", visitentity);
        ...
        public virtual async Task InsertAppointmentCreditBillAsyncSP(Registration objRegistration, VisitDetail objVisitDetail, Bill objBill, Payment objPayment, List<AddCharge> ObjaddCharge, int CurrentUserId, string currentUserName)
        {
            ...
            try
            {
                ...
                string[] rEntity = { "RegDate", "RegTime", "PrefixId", "FirstName", "MiddleName", "LastName", "Address", "City", "PinNo", "DateofBirth", "Age", "GenderId", "PhoneNo", "MobileNo", "AddedBy", "AgeYear", "AgeMonth", "AgeDay", "CountryId", "StateId", "CityId", "MaritalStatusId", "IsCharity",
                    "ReligionId","AreaId","IsSeniorCitizen","AadharCardNo","PanCardNo","Photo","EmgContactPersonName","EmgRelationshipId","EmgMobileNo","EmgLandlineNo","EngAddress","EmgAadharCardNo","EmgDrivingLicenceNo","MedTourismPassportNo","MedTourismVisaIssueDate","MedTourismVisaValidityDate","MedTourismNationalityId","MedTourismCitizenship","MedTourismPortOfEntry","MedTourismDateOfEntry","MedTourismResidentialAddress","MedTourismOfficeWorkAddress","EmailId","RegId","MembershipId","AbhaTranId","IsMember" };
                var entity = objRegistration.ToDictionary();
                foreach (var rProperty in entity.Keys.ToList())
                {
                    if (!rEntity.Contains(rProperty))
        ...
        public virtual async Task RegisteredAppBillInsert(Registration objRegistration, VisitDetail objVisitDetail, Bill objBill, Payment objPayment, List<AddCharge> ObjaddCharge, int CurrentUserId, string CurrentUserName)
        {
            await using var transaction = await _context.Database.BeginTransactionAsync();
            ...
            try
            {
                ...
                odal.ExecuteNonQueryNew("ps_update_Registration_1", CommandType.StoredProcedure, "", entity);
        ...
        public virtual async Task InsertAsyncTDrbill(TDrbill ObjTDrbill, List<TDrbillDet> ObjTDrbillDet, List<TDraddCharge> ObjTDraddCharge, int CurrentUserId, string CurrentUserName)
        {
            ...
            try
            {
                ...
                string VDrbno = odal.ExecuteNonQueryNew("PS_Insert_T_DRBill", CommandType.StoredProcedure, "Drbno", bentity);
