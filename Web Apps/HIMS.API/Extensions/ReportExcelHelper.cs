    ...
    public static class ReportExcelHelper
    {
        ...
        "Cash","UPI","Card","User"
                ...
                case "SalesWiseCollectionSummary.html":
                    ...
                    break;
                case "SalesWiseCollectionSummary_New.html":
                    {
                        int rowNo = 2;
                        int srNo = 1;
                        string[] headers =
                            {
                            "Sr.No","PaymentDate","SalesDate","SalesNo","PatientName",
                          "TotalAmt","DiscAmt","NetAmt","PaidAmt",
                            "BalAmt","CashPay","ChequePay","NeftPay","PayTMPay","CardPay","Mpesa Ref No"
                        };
                        ...
                        decimal gtTot = 0, gtDisc = 0, gtNet = 0, gtPaid = 0, gtBal = 0;
                        decimal gtCash = 0, gtCheque = 0, gtNeft = 0, gtPaytm = 0, gtCard = 0;
                        ...
                        foreach (var grp in groups)
                        {
                            ...
                            foreach (var dr in grp)
                            {
                                string mpesa = dr["TransactionRefNo"]?.ToString() ?? "";
                                workSheet.Cell(rowNo, 1).Value = srNo++;
                                workSheet.Cell(rowNo, 2).Value =
                               dr["PaymentDate"] == DBNull.Value
                               ? ""
                               : Convert.ToDateTime(dr["PaymentDate"]).ToString("dd/MM/yyyy");
                                ...
                                workSheet.Cell(rowNo, 6).Value = Dec(dr, "TotalAmount");
                                workSheet.Cell(rowNo, 7).Value = Dec(dr, "DiscAmount");
                                workSheet.Cell(rowNo, 8).Value = Dec(dr, "NetAmount");
                                workSheet.Cell(rowNo, 9).Value = Dec(dr, "PaidAmount");
                                workSheet.Cell(rowNo, 10).Value = Dec(dr, "BalanceAmount");
                                workSheet.Cell(rowNo, 11).Value = Dec(dr, "CashPay");
                                workSheet.Cell(rowNo, 12).Value = Dec(dr, "ChequePay");
                                workSheet.Cell(rowNo, 13).Value = Dec(dr, "NeftPay");
                                workSheet.Cell(rowNo, 14).Value = Dec(dr, "PayTMPay");
                                workSheet.Cell(rowNo, 15).Value = Dec(dr, "CardPay");
                                workSheet.Cell(rowNo, 16).Value = mpesa;
                            ...
                            workSheet.Cell(rowNo, 1).Value = $"Total : {grp.Key}";
                            workSheet.Cell(rowNo, 6).Value = stTot;
                            workSheet.Cell(rowNo, 7).Value = stDisc;
                            workSheet.Cell(rowNo, 8).Value = stNet;
                            workSheet.Cell(rowNo, 9).Value = stPaid;
                            workSheet.Cell(rowNo, 10).Value = stBal;
                            workSheet.Cell(rowNo, 11).Value = stCash;
                            workSheet.Cell(rowNo, 12).Value = stCheque;
                            workSheet.Cell(rowNo, 13).Value = stNeft;
                            workSheet.Cell(rowNo, 14).Value = stPaytm;
                            workSheet.Cell(rowNo, 15).Value = stCard;
                            workSheet.Cell(rowNo, 16).Value = "";
                        ...
                        workSheet.Cell(rowNo, 1).Value = "Grand Total";
                        workSheet.Cell(rowNo, 6).Value = gtTot;
                        workSheet.Cell(rowNo, 7).Value = gtDisc;
                        workSheet.Cell(rowNo, 8).Value = gtNet;
                        workSheet.Cell(rowNo, 9).Value = gtPaid;
                        workSheet.Cell(rowNo, 10).Value = gtBal;
                        workSheet.Cell(rowNo, 11).Value = gtCash;
                        workSheet.Cell(rowNo, 12).Value = gtCheque;
                        workSheet.Cell(rowNo, 13).Value = gtNeft;
                        workSheet.Cell(rowNo, 14).Value = gtPaytm;
                        workSheet.Cell(rowNo, 15).Value = gtCard;
                        workSheet.Cell(rowNo, 16).Value = "";
