...
namespace HIMS.API.Controllers.Masters.Personal_Information
{
    [Route("api/v{version:apiVersion}/[controller]")]
        ...
        {
            private readonly IItemMasterService _IItemMasterService;
            public ItemMasterController(IItemMasterService repository)
            {
            _IItemMasterService = repository;
            }
            ...
            public async Task<ApiResponse> Insert(ItemMasterModel obj)
            {
                ...
                model.Addedby = CurrentUserId;
                    await _IItemMasterService.InsertAsync(model, CurrentUserId, CurrentUserName);
                }
                else
            ...
            //        await _IItemMasterService.Update(model, CurrentUserId, CurrentUserName, new string[2] { "CreatedBy", "CreatedDate" });
            //    }
            ...
            //{
            //    MItemMaster model = await _IItemMasterService.GetById(x => x.ItemId == Id);
            //    if ((model?.ItemId ?? 0) > 0)
            //    {
            //        model.Isdeleted = false;
            //        model.UpDatedBy = CurrentUserId;
            //        model.IsUpdatedBy = DateTime.Now;
            //        await _IItemMasterService.SoftDelete(model, CurrentUserId, CurrentUserName);
