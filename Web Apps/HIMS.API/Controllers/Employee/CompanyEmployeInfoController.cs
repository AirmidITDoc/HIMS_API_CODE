...
namespace HIMS.API.Controllers.Employee
{
    ...
    public class CompanyEmployeInfoController : BaseController
    {
        ...
        public async Task<ApiResponse> GetDropdown()
        {
            ...
            var CompanyEmployeInfoList = await _repository.GetAll( x => x.IsActive == true && x.IsRepresentative == false);
            return ApiResponseHelper.GenerateResponse(ApiStatusCode.Status200OK, " Employee dropdown", CompanyEmployeInfoList.Select(x => new { FullName = x.FirstName + " " + x.LastName , x.ExecutiveId }));
        }
        //List API
        [HttpGet]
        ...
        public async Task<ApiResponse> GetIsRepresentativeDropdown()
        {
            ...
            return ApiResponseHelper.GenerateResponse(ApiStatusCode.Status200OK, "Representative dropdown", CompanyEmployeInfoList.Select(x => new { FullName = x.FirstName + " " + x.LastName, x.ExecutiveId }));
        }
