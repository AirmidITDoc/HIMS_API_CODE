...
namespace HIMS.ScheduleJobs.Logic
{
    public class SMSLogic
    {
        ...
        {
            DataTable dt = await DatabaseHelper.FetchDataTableAsync("GET_SMS_DATA", Array.Empty<SqlParameter>());
            foreach (DataRow row in dt.Rows)
            {
                try
                {
                    ...
                    await DatabaseHelper.IUD("UPDATE T_SMS_Outgoing SET Status=@Status,LastTry=GETDATE(),LastResponse=@LastResponse WHERE SmsOutGoingId=@Id", parameters);
                }
                catch (Exception ex)
                {
